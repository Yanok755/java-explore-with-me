...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("SELECT new ru.practicum.ewm.stats.dto.ViewStatsDto(e.app, e.uri, COUNT(e)) " +
            ...
            "GROUP BY e.app, e.uri ORDER BY COUNT(e) DESC")
    List<ViewStatsDto> findStatsByUris(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);
    ...
    @Query("SELECT new ru.practicum.ewm.stats.dto.ViewStatsDto(e.app, e.uri, COUNT(DISTINCT e.ip)) " +
            ...
            "WHERE e.timestamp BETWEEN :start AND :end AND e.uri IN :uris " +
            "GROUP BY e.app, e.uri ORDER BY COUNT(DISTINCT e.ip) DESC")
    List<ViewStatsDto> findUniqueStatsByUris(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("uris") List<String> uris);
}
